...
rs_synth <- function(var_new, resampling, Y,
                     id_treat, id_control, corona_subsample2.df, 
                     predictors, predictors.op, time.predictors.prior,
                     special.predictors, dependent, t_pre, end,
                     lp, bwdating){
  
  Y <- corona_subsample2.df[which(corona_subsample2.df$iso_code_num == id_treat), dependent]
  
  ### new predictor vars
  new_p <- predictors[var_new[which(var_new <= lp)]]
  new_sp <- special.predictors[(var_new[which(var_new > lp)] - lp)]
  ...
  tryCatch({ # Avoid stopping because of singularities
    ...
    dataprep_outbs <- dataprep(
      ...
      time.plot = min(corona_subsample2.df$date_num):max(corona_subsample2.df$date_num)
    )
...
synth_fun <- function(data = NULL, treat.date = NULL, treat.id = NULL,
                      ...
                      resampling = 0.8,
                      Rbs = 10, alpha = 0.1, seed = 24, workers = availableCores()){
  ...
  varlist  <- vector(mode = "list", length = Rbs)
  
  lp <- length(predictors)
  lsp <- length(special.predictors)
  l <- length(c(predictors, special.predictors))
  
  NC <- round(l * resampling, 0)
  
  for(i in 1:Rbs){
    varlist[[i]] <- sample(1:l, NC, replace = FALSE)
  }  
  
  corona_subsample2.df <- corona_subsample2.df[which(corona_subsample2.df$date_num %in% as.numeric(rownames(Y))), ]
  
  # Bootstrap parallel
  plan(multisession, workers = workers)
  
  bs.res <- future_lapply(varlist, function(x) rs_synth(x, resampling, Y,
                                                        id_treat, id_control, corona_subsample2.df, 
                                                        predictors, predictors.op, time.predictors.prior,
                                                        special.predictors, dependent, t_pre, end,
                                                        lp, bwdating))
...
### Control group (Exlcude US for isr)
isos <- c("AUT", "BEL", "CAN", "CZE", "ESP", "FIN", "GBR", "GRC", "HRV", "IRL", "LTU", "LUX", "NLD", "POL", "PRT", "SVK", "SVN", "SWE", "USA")

isos_isr <- c("AUT", "BEL", "CAN", "CZE", "ESP", "FIN", "GBR", "GRC", "HRV", "IRL", "LTU", "LUX", "NLD", "POL", "PRT", "SVK", "SVN", "SWE")
...
plot.list1_1 <- synth_fun(data = corona_subsample.df,
                        ...
                        Rbs = 1000, seed = 58)
Sys.time() - t
save(plot.list1_1, file = "resamp_plot_list1_1.RData")
...
pdf(file = paste0("../03_Output/resample_", "Vaccinations_Synthetic_", name, ".pdf"), width = 12, height = 6)
...
png(file = paste0("../03_Output/resample_", "Vaccinations_Synthetic_", name, ".png"), width = 12, height = 6, units = "in", res = 300)
grid.arrange(grobs = plot.list1_1, ncol = 2,
             # bottom = g,
             top = textGrob(paste0("COVID health certificates: ", name), gp = gpar(fontsize = 16))
             )
...
save(plot.list2_1, file = "resamp_plot_list2_1.RData")
...
save(plot.list3_1, file = "resamp_plot_list3_1.RData")
...
save(plot.list4_1, file = "resamp_plot_list4_1.RData")
...
save(plot.list5_1, file = "resamp_plot_list5_1.RData")
...
save(plot.list6_1, file = "resamp_plot_list6_1.RData")
